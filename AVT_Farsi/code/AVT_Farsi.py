...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='G:\\Ghazaleh Khosravi\\Task Design Psychopy\\AVT_Farsi\\code\\mYousefi.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    back3 = visual.Rect(
        ...
        colorSpace='rgb', lineColor='black', fillColor='black',
        opacity=1.0, depth=0.0, interpolate=True)
    Other_Text = visual.ImageStim(
        win=win,
        name='Other_Text', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0.35), draggable=False, size=(1, 1),
        color=[1,1,1], colorSpace='rgb', opacity=1.0,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    ...
    back6 = visual.Rect(
        ...
        colorSpace='rgb', lineColor='black', fillColor='black',
        opacity=1.0, depth=0.0, interpolate=True)
    Quest = visual.ImageStim(
        win=win,
        name='Quest', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(0.85, 0.85),
        color=[1,1,1], colorSpace='rgb', opacity=1.0,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    ...
    Press_Space_2 = visual.ImageStim(
        win=win,
        name='Press_Space_2', 
        image='pressSpace.jpg', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(0.7, 0.6),
        color=[1,1,1], colorSpace='rgb', opacity=1.0,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    ...
    Thank_You = visual.ImageStim(
        win=win,
        name='Thank_You', 
        image='appreciaition.jpg', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1, 1),
        color=[1,1,1], colorSpace='rgb', opacity=1.0,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    ...
    PractPhase = data.Routine(
        name='PractPhase',
        components=[back8, Press_Space, Resp4Space],
    )
    ...
    while continueRoutine:
        ...
        if Resp4Space.status == NOT_STARTED and tThisFlip >= 10.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    for thisPractTrial in PractTrial:
        ...
        taskT1 = data.Routine(
            name='taskT1',
            components=[back3, Other_Text, Pic, Vc],
        )
        taskT1.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        Other_Text.setImage('otherListentingTo.jpg')
        Pic.setImage(image)
        ...
        while continueRoutine:
            ...
            # if Other_Text is starting this frame...
            if Other_Text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                Other_Text.frameNStart = frameN  # exact frame index
                Other_Text.tStart = t  # local t and not account for scr refresh
                Other_Text.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(Other_Text, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Other_Text.started')
                # update status
                Other_Text.status = STARTED
                Other_Text.setAutoDraw(True)
            
            # if Other_Text is active this frame...
            if Other_Text.status == STARTED:
                # update params
                pass
            
            # if Other_Text is stopping this frame...
            if Other_Text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Other_Text.tStartRefresh + duration-frameTolerance:
                    # keep track of stop time/frame for later
                    Other_Text.tStop = t  # not accounting for scr refresh
                    Other_Text.tStopRefresh = tThisFlipGlobal  # on global time
                    Other_Text.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Other_Text.stopped')
                    # update status
                    Other_Text.status = FINISHED
                    Other_Text.setAutoDraw(False)
        ...
        Quest.setImage(soq)
        RatingResp.reset()
        # Run 'Begin Routine' code from code_mouse_Visible
        win.mouseVisible = True
        ...
        # Run 'End Routine' code from code_mouse_Visible
    ...
    instruction = data.Routine(
        name='instruction',
        components=[background, Press_Space_2, Resp1Space],
    )
    ...
    BreakRoutine = data.Routine(
        name='BreakRoutine',
        components=[back5, Press_Break, Resp2Space],
    )
    ...
    while continueRoutine:
        ...
        # if Press_Break is starting this frame...
        if Press_Break.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Press_Break.frameNStart = frameN  # exact frame index
            Press_Break.tStart = t  # local t and not account for scr refresh
            Press_Break.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Press_Break, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Press_Break.started')
            # update status
            Press_Break.status = STARTED
            Press_Break.setAutoDraw(True)
        
        # if Press_Break is active this frame...
        if Press_Break.status == STARTED:
            # update params
            pass
    ...
    Finish = data.Routine(
        name='Finish',
        components=[back7, Thank_You, Resp3Space],
    )
